...
# **Enforcement Directives for Cline and MCP Server**

!!! ATTENTION: Memory Bank Integration Enforcement
These directives **must be followed** by Cline and the Sightline MCP server to ensure persistent, explicit, and reliable memory management.
!!!

## **Memory Bank Loading**

- **Always load** the following files at startup:
  - `memory_docs/codeMap_root.md`
  - `memory_docs/activeContext.md`
  - `memory_docs/indexes/*.yaml`
- **Load additional context** only as needed, respecting paging protocols.

## **Memory Bank Updating**

- **After every MCP tool invocation** (snapshot, validate, diff, etc.):
  - **Update relevant YAML indexes** with new snapshot metadata, validation results, diff info.
  - **Append or update markdown docs** (`progress.md`, snapshot-specific notes) with results and decisions.
  - **Log all tool calls and outcomes** explicitly.

## **Paging and Context Activation**

- **Activate only essential components** for the current task.
- **Page out** unused context to maintain efficiency.
- **Request additional context** explicitly when needed.

## **Explicit Tool Invocation**

- **Always prefer** explicit MCP tools over implicit actions.
- **Request explicit user approval** before destructive actions.
- **Chain tool calls** explicitly, logging each step.

## **Logging and Traceability**

- **Log all decisions, tool calls, and context changes** in the Memory Bank.
- **Maintain audit trails** in markdown files.
- **Update progress.md** continuously.

## **Error Handling**

- **Log all errors** with context and attempted fixes.
- **Request clarification** from the user when uncertain.
- **Retry or suggest alternatives** as needed.

## **Summary**

- The Memory Bank is the **single source of truth**.
- Cline and the MCP server **must load, update, and respect** it at all times.
- This ensures **persistent, explicit, queryable memory** across sessions.
- **No operation** should bypass Memory Bank integration.
